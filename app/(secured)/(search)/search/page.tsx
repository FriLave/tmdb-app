...
export default function SearchPage() {
    ...
    const { data, hasNextPage, isFetchingNextPage, fetchNextPage} = useInfiniteQuery({
        ...
        queryFn: async ({pageParam = 1, queryKey}) => {
            ...
            const res = await httpClient.get<ApiPaginatedResponse<Movie | Serie>>(`/api/search?page=${pageParam}&query=${query}`);
            return res.data;
        },
    ...
    return (
        <div className={"flex flex-col gap-4"}>
            <h1 className={'text-2xl'}>Résulats de recherche pour <strong>{query}</strong>...</h1>
            <InfiniteScrollLayout hasNextPage={hasNextPage} isFetchingNextPage={isFetchingNextPage} fetchNextPage={fetchNextPage}>
                        ...
                        <React.Fragment key={i}>
                            {
                                page?.results?.map((media) => {
                                    ...
                                    const title = isMovie ? (media as Movie).title : (media as Serie).name;
                                    const releaseDate = isMovie ? (media as Movie).release_date : (media as Serie).first_air_date;
